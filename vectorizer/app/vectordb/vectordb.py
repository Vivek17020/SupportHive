...
class VectorDB:
    ...
    def create_or_clear_collection(self):
        ...
        self.client.create_collection(
            ...
            vectors_config=VectorParams(size=1024, distance=Distance.COSINE)  # Adjust size as per OLMo
        )
    ...
    async def index_regular_docs(self):
        ...
        logger.info(f"Finished indexing. Total documents indexed into {self.collection_name}: {len(chunks)}")
    ...
    def search(self, query, limit=2, with_payload=True):
        query_vector = generate_embedding(query)  # This should use OLMo as well
